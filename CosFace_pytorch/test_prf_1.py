...
def img_save(file_path, img):
    if '/dataset/V1/' in file_path:
        new_file_path = file_path.replace('/dataset/V1/', '/dataset/V1_new/')
        img_dir = '/'.join(new_file_path.split('/')[:-1])
        if not os.path.exists(img_dir):
            os.system('mkdir {}'.format(img_dir)) 
        img.save(new_file_path) 
    else:
        new_file_path = file_path.replace('/dataset/actor_imgs_multi_0910_test300/', '/dataset/actor_imgs_multi_0910_test300_new/')
        img_dir = '/'.join(new_file_path.split('/')[:-1])
        if not os.path.exists(img_dir):
            os.system('mkdir {}'.format(img_dir)) 
        img.save(new_file_path)
    ...
    if len(bounding_boxes) == 0 or len(landmarks) == 0:
        ...
        try:
            ...
            if len(bounding_boxes) == 0 or len(landmarks) == 0:
                ...
                img = image.crop(bounding_boxes[0][0:4]).resize((96, 112), Image.BILINEAR)
            img_save(file_path, img)
            return np.array([1,2,3])
        dst_list = []
        try:
